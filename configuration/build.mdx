...
description: Configure content sources, output paths, and other build options in Maizzle
---
...
<ParamField path="content" type="string[]" default="['emails/**/*.html']">
...
<Note>Previously this was called 'Function source', and it allowed defining sources as a function that was evaluated by Maizzle. This is deprecated starting with Maizzle 5.</Note>
...
<ParamField path="output" type="object" default="{ path: 'build_[env]', extension: 'html', from: ['emails'] }">
  Define the output path for compiled Templates, and what file extension they should use.
</ParamField>

### path

<ParamField path="output.path" type="string" default="build_[env]">
...
<ParamField path="output.extension" type="string" default="undefined">
...
<ParamField path="output.from" type="string[]" default="['emails']">
  ...
  <Warning>You must specify all directories to unwrap when using `output.from` and multiple `build.content` source paths, as this option overwrites the default `['emails']` value.</Warning>
...
<ParamField path="permalink" type="string" default="undefined">
  ...
  <Warning>`permalink` must be a <em>file</em> path, and can only be used in the Template's Front Matter. Using a directory path will result in a build error.</Warning>
</ParamField>

## static

<ParamField path="static" type="object" default="{ source: '', destination: 'assets' }">

...
In this tutorial, we'll use [Events](/development/events) in Maizzle to fetch the contents of an RSS feed and display them in an HTML email newsletter.
...
We can use `rss-parser` inside the [beforeCreate](/development/events#beforecreate) event to fetch feed data.
...
<Note>
The Laracasts feed contains fields that `rss-parser` does not currently return by default. We include them through the `customFields` option.
</Note>
...
<Note>
Tip: you could set `'en-US'` dynamically, based on your subscriber's preference.
</Note>
...
- [Maizzle Events](/development/events/)

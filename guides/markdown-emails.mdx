...
<Note>
You can remove the `emails` directory, we won't need it.
</Note>
...
- `front-matter` to be able to use [Front Matter](/development/templates#front-matter) in our Markdown files
...
We can use the [`beforeRender` event](/development/events#beforerender) for this:
...
3. We're returning a string that includes the contents of the `body` property wrapped in `<md>` tags, so Maizzle can parse them as Markdown. See the [Markdown documentation](/development/markdown) for more info on this tag. Finally, the `<x-main>` tag tells Maizzle to use our `main.html` layout.
...
```html build_production/newsletter-1.html
...
You can use [expressions](/development/templates#expressions) in Markdown files just as you would in any Maizzle template:
...
- Docs for [Markdown in Maizzle](/development/markdown)

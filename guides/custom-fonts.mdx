---
title: Using custom web fonts in email templates
...
It's super easy to [use Google Fonts in your Maizzle email templates](/examples/google-fonts), but what if you need to use a custom web font?
...
With [CSS inlining](/transformers/inline-css) enabled, that would result in:
...
We can now use it on the outermost element:
...
<Note>
Don't add it to the `<body>` - some email clients remove or replace this tag.
</Note>
...
Since Outlook on Windows doesn't read `@media` queries, define a fallback for it in the `<head>` of your Layout:
...
<Note>
The Maizzle Starter includes this fallback in the `main.html` Layout by default.
</Note>

...
<Note>
`_embed=1` is a request scope that adds a few more fields to the response. We use it to include `_embedded["wp:featuredmedia"]`.
</Note>
...
Want to loop over a specific subset only? You can use [expressions](/development/templates#expressions).
...
- we're using [`loop` meta](/development/tags#loop-meta) to output the divider only _between_ list items

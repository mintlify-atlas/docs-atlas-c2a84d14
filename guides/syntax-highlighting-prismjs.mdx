...
<Note>
Notice how we added the `language-javascript` class on the `<code>` tag - this is required in order to get language-specific syntax highlighting.
</Note>

<Warning>
You need to reset the indentation of code inside the `<pre>` tag yourself - see the [PostHTML example](https://github.com/maizzle/example-syntax-highlight/blob/master/emails/posthtml.html) in the tutorial repository.
</Warning>

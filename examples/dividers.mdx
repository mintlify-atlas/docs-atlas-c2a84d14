...
<Note>
Note that `<hr>` elements come with margins by default, so you might want to set a custom one or reset it with `m-0`. For example, Chrome resets to `8px`.
</Note>
...
The Maizzle Starter includes a [Divider component](/components/divider) that uses the `<div>` technique.

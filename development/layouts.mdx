...
A Layout is basically a [Component](/development/components) that contains the `doctype`, `<head>` and `<body>` tags of your HTML - the kind of code that changes rarely and can be reused.
...
<Note>Need to store them elsewhere? Make sure to [update the config](/configuration/components#folders).</Note>
...
<Note>For Tailwind CSS to work, Layouts must include it either via a `<style>` tag like above or through a `<link>` tag that references a CSS file.</Note>
...
Read more about this in the [Components docs](/development/components#x-tag).
...
<Warning>We don't recommend using `@tailwind base` because it contains CSS resets that won't work or are not needed in HTML emails. Also, some resets in there use the `*` selector, which can cause issues when CSS is inlined.</Warning>
...
<Note>Make sure to include the `inline` attribute on the `<link>` tag, this will replace it with a `<style>` tag that can be inlined and is generally better supported in email clients.</Note>
...
Variables from your [Environment config](/configuration/environments) or from the Template's own Front Matter are available in a Layout under the `page` object.

You can use the curly braces [expression syntax](/development/expressions) to output variables in a Layout:

...
<Warning>The `src` attribute is reserved on Components, make sure not to use it as a [prop](#props) name.</Warning>
...
<Warning>`stack` and `push` tags require a non-empty `name` attribute.</Warning>
...
<Warning>Only CommonJS syntax with `module.exports` is currently supported in Components.</Warning>
...
[Expressions](/development/expressions) may be used in a Component's attribute:
...
If you need to safelist or even block certain HTML attributes that you pass to a Component, see the [Component configuration docs](/configuration/components#elementattributes).

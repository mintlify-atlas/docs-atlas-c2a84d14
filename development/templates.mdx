...
To output them in a Template, use the `{{ }}` [expression syntax](/development/expressions):
...
<Warning>Front Matter must be defined at the very top of a Template, starting on the first line.</Warning>
...
To reuse this code in Maizzle, you may create a [Layout](/development/layouts):
...
Learn more about how these x-tags work, in the [Components docs](/development/components#x-tag).
...
<Note>Current template file information is not available when using the [API](/api).</Note>
...
<Steps>
  <Step title="Move to another directory">
    Move them to a directory outside the ones defined in `build.content`
  </Step>
  <Step title="Change file extension">
    Change their file extension so that it's not covered by paths from `build.content`
  </Step>
  <Step title="Use negated glob patterns">
    Use negated glob patterns in `build.content` to exclude them.

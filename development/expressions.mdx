...
Handlebars-like, curly brace expression syntax is supported, allowing you to access variables from your [Environment config](/configuration/environments) or from a Template's Front Matter:
...
<Warning>Maizzle components, like `<x-button>`, are not ignored inside `<raw>` and will be compiled as usual.</Warning>

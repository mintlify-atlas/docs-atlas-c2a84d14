...
<Warning>
Tailwind CSS utilities must be passed inside the `class` attribute, not the `color` attribute.
</Warning>
...
<Warning>
Margin props will override `space-y|x` props.
</Warning>
...
<Warning>
`space-y|x` props will be overridden by individual margin props.
</Warning>
...
There are other ways to create horizontal visual dividers in your HTML emails, such as using a `<table>` or an `<hr>`. Check out our [Divider examples](/examples/dividers) for more ideas.

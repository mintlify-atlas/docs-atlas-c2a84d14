...
If you're repeating the same utility classes over and over again, you can extract them to a [Component](/development/components) so you've got a single source of truth and can make changes in one place.
...
<Note>When importing CSS files you need to use the path relative to the root of your project's directory.</Note>

<Warning>`@import` statements need to come before any other CSS rules in the `<style>` tag, otherwise the entire CSS inside them will be discarded.</Warning>

## Shorthand CSS

<Note>This section refers to writing shorthand CSS inside `<style>` tags. For _inline_ CSS shorthand, see the [Shorthand CSS Transformer docs](/transformers/shorthand-css).</Note>
...
<Note>For shorthand CSS to work with `padding` or `margin`, you need to specify property values for all four sides. For borders, keep reading.</Note>
...
<Note>`tailwindcss-email-variants` is already included in our Tailwind CSS preset, you don't need to install it separately.</Note>
...
Although it will no longer be inlined, unused CSS will still be purged - see the [css.purge](/transformers/purge-css) Transformer docs for more information.
...
<Note>As you can see, this also requires that you add a `body` class on your `<body>` tag.</Note>
